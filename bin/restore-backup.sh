...
docker cp -q "themes/" $containerPath/
docker cp -q "var/" $containerPath/
docker cp -q "web/" $containerPath/
docker cp -q "config.php" $containerPath/
docker cp -q ".htaccess" $containerPath/
docker cp -q ".htpasswd" $containerPath/

docker exec $swContainer bash -c "chown -R www-data:www-data $swDir"
docker exec --user www-data $swContainer bash -c "chmod -R 0777 $swDir"
docker exec --user www-data $swContainer bash -c "cd $swDir && bin/console sw:cache:clear"
